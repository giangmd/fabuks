...
class SuperUpdate extends Command {
	...
	public function handle() {
		...
		if (empty($admin)) {
			// create admin
			if (!$options['create']) {
				$this->warn('This super-manager not found. 
                    Please add more option {--create} to create new super-manager.');
				exit();
			} else {
				$user = User::create([
	...
	protected function validateArguments($arguments) {
		...
		if (empty($arguments['email'])
			|| empty($arguments['password'])
			|| empty($arguments['password_confirmation'])) {
			$response['message'] = "Email and Password is required.";
			return $response;
		}

		if (!$this->isEmail($arguments['email'])) {
			$response['message'] = trans('validation.email', ['attribute' => 'email']);
			return $response;
		}

		if (strlen($arguments['password']) < 6) {
			$response['message'] = trans('validation.min.numeric', [
				'attribute' => 'Password',
				'min' => 6
			]);
			return $response;
		...
		if ($arguments['password'] !== $arguments['password_confirmation']) {
			$response['message'] = trans('validation.same', [
					'attribute' => 'Password',
					'other' => 'Password Confirmation'
				]);
			return $response;
		}
